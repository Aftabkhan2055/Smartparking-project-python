...
class combinder:
    ...
    def __init__(self):
        ...
        option2=Menu(mymenu)
        mymenu.add_cascade(label="sell",menu=option2)

        option2.add_command(label="Today sell",command=self.test6)
        option2.add_command(label="From date and to date",command=self.test7)
        option2.add_command(label="total price aacording vehicle no",command=self.test8)


        option2=Menu(mymenu)
        mymenu.add_cascade(label="admin",menu=option2)

        option2.add_command(label="Add ADMIN",command=self.test9)
        option2.add_command(label="View Admin",command=self.test11)
        option2.add_command(label="Update/Delete Admin",command=self.test10)
